...
export const FootballResults = defineStore("football-results", () => {
  const MIN_VALUES_TO_BE_PRESENTED = 5;
  const VALUES_TO_ADD_PER_CLICK = 3;
  ...
  const footballFirstLeagueMainPositions =
    ref<FootballResultsList[]>(defaultValue);
  ...
  const getAvailableCountries = computed<CountryInformation[]>(
  ...
  const getActualCountryInfo = computed(
    () =>
      availableCountries.value.find(
        (country) => country.id === actualCountry.value
    ...
    if (fullResultsDisplayed.value) {
      if (nextIndex >= footballFirstLeagueMainPositions.value.length) {
        footballFirstLeagueMainPositionsToShow.value =
          footballFirstLeagueMainPositions.value;
        ...
        footballFirstLeagueMainPositionsToShow.value =
          footballFirstLeagueMainPositions.value.slice(0, nextIndex);
        nextIndex += VALUES_TO_ADD_PER_CLICK;
      }
    } else {
      //hide full results
  ...
  async function changeCountry(event: { id: string }) {
  ...
  async function getFootballFirstLeaguePositions() {
    const countryInfo = availableCountries.value.find(
      (country) => country.id === actualCountry.value
    ) as CountryInformation;

    const returnValues = await window
      .fetch(
        `https://www.thesportsdb.com/api/v1/json/3/lookuptable.php?l=${countryInfo.leagueId}&s=${countryInfo.availableSeason}`,
        {
          method: "GET",
        }
      )
    ...
    footballFirstLeagueMainPositions.value = returnValues.table.map(
      (result: FootballResultsList) => {
